...
(function() {
  var Ajax, Invocation, Invoker, ServerSideClass, Signal, exports, getClass,
    __slice = [].slice,
  ...
  Invoker = (function() {
    ...
    Invoker.adding_invocation = new Signal;

    Invoker.adding_callback = new Signal;

    Invoker.url = 'gateway';

    function Invoker(classname, methods, __construct_args) {
      var method, _i, _len;
      this.classname = classname;
      this.__construct_args = __construct_args;
      for (_i = 0, _len = methods.length; _i < _len; _i++) {
        method = methods[_i];
        this.__register(method);
      }
      this.__counter = 0;
    }

    Invoker.prototype.__register = function(method) {
      return this[method] = this.__call(method);
    };

    Invoker.prototype.__call = function(method) {
      return function() {
        var args, call, handled, id,
          _this = this;
        args = 1 <= arguments.length ? __slice.call(arguments, 0) : [];
        call = [this.classname, method, args, this.__construct_args];
        id = this.__counter;
        this.__counter += 1;
        handled = this.constructor.adding_invocation.dispatch([call, id, this]);
        if (handled) {
          return (function(id) {
            return function(cb) {
              return _this.constructor.adding_callback.dispatch([cb, id, _this]);
            };
          })(id);
        } else {
          return function(cb) {
            var invocation;
            invocation = new Invocation(_this.constructor.url, [call], [cb]);
    ...
    Invoker.invoke = function(_arg, cb) {
      var args, classname, construt_args, invocation, method;
      classname = _arg[0], method = _arg[1], args = _arg[2], construt_args = _arg[3];
      invocation = new Invocation(this.url, [[classname, method, args, construt_args]], [cb]);
      return invocation.send();
    };

    Invoker.batch = function(setup) {
      ...
      calls = [];
      registration = this.adding_invocation.add(function(_arg) {
        var call, id, invoker;
        call = _arg[0], id = _arg[1], invoker = _arg[2];
        calls.push([call, id, invoker]);
        return true;
      });
      registration2 = this.adding_callback.add(function(_arg) {
      ...
      setup(done, map);
      this.adding_invocation.remove(registration, registration2);
      callbacks = (function() {
        var _i, _len, _ref, _results;
        _results = [];
      ...
      invocation = new Invocation(this.url, calls, callbacks);
      return invocation.send(done_callback, map_callback);
    };

    Invoker.abort = function() {};
  ...
  Invocation = (function() {
    ...
    function Invocation(url, __calls, __callbacks) {
      var _ref, _ref1;
      this.url = url;
  ...
  ServerSideClass = (function() {

    function ServerSideClass() {
      var construct_args;
      construct_args = 1 <= arguments.length ? __slice.call(arguments, 0) : [];
  ...
  getClass = function() {
    var class, cls, methods, _class, _i, _len;
    _class = arguments[0], methods = 2 <= arguments.length ? __slice.call(arguments, 1) : [];
    this["class"] = _class;
    cls = (function(_super) {
